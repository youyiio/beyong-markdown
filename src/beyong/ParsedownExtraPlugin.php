...
class ParsedownExtraPlugin extends ParsedownExtra {
    ...
    // custom header h1,2,3... class for class name without dot prefix
    ...
    // storage header title value[hx,text,id]
    ...
    protected function blockHeader($Line)
    {
        ...
        if ($this->header_class) {
            $block['element']['attributes']['class'] = $this->header_class;
            $block['element']['attributes']['id'] = uniqid(time());
        }
        //存储headers,[h1,text,id]
        array_push($this->headers, [$block['element']['name'], $block['element']['text'], $block['element']['attributes']['id']]);
